...
│  └── Timeout → query core "does customer exist for phone?"    │
│               ├── YES → continue with returned CIF            │
│               └── NO  → retry step 2                          │
│                                                               │
...
Saga state is stored in the `onboarding_applications` table (a `saga_state` column) so it survives restarts.

---

## 5. Impedance Mismatch: REST ↔ SOAP
...
## 9. What's TBD (Needs Bank's WSDLs)
